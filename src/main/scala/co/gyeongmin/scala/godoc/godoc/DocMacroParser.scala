...
import co.gyeongmin.scala.godoc.godoc.doc.elements.{ArgList, Text, TypeRequired}
import co.gyeongmin.scala.godoc.golang.GoParser
import co.gyeongmin.scala.godoc.golang.syntax.{GoFunctionDef, GoSomeDef, GoTypeDef}
...
object DocMacroParser {
  ...
  def parseComment(str: String): IncompleteBlock = {
    def recur(it: Iterator[String], acc: IncompleteBlock): IncompleteBlock = {
      if (!it.hasNext) acc
        ...
        recur(it, key match {
          ...
          case "implements" => acc.copy(interface = Text.parse(value.dropWhile(_ == ':').trim).txt.split(",").map(_.trim).toList)
    ...
    }, IncompleteBlock(TypeRequired.empty, Nil, "", "", builder = false))
  }
  ...
  case class IncompleteBlock(args: TypeRequired, interface: List[String], desc: String, ex: String, builder: Boolean) {
    ...
    def resolveType: PartialFunction[GoSomeDef, DefBlock] = {
      ...
      case x: GoTypeDef => TypeDefBlock(x.typeName, x.isInterface, interface, desc.toOption, ex.toOption, builder)
    }
  }

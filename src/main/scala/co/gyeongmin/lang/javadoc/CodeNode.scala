...
final case class CodeLeaf(
    name: String,
    packageName: String,
    tokens: List[JavaSToken],
    outputDir: String,
  ...
  private val reformatPw: PrintWriter = new PrintWriter(
    s"$outputDir/$relativePath/$name.html"
  )
  reformatPw
    .write(
      """<!DOCTYPE html><html><body><pre style="background: black; color: #BCBCBC; overflow-wrap: normal;">"""
    )
  reformatPw.write(
    JavaCodeFormatter.printCode(name, tokens.toVector, debugOption) match {
  ...
  override def print[T]: Node[T] =
    div(`class` := "panel", hr(), div(code.show(debugOption)))
  ...
  override def buildNavTree[T]: Node[T] = {
    ...
    def recur[U](definition: JavaDefinition): Node[U] = {
      definition match {
        case JavaClass(_, modifier, definitions, _, _)
            if modifier.access == PUBLIC =>
          li(
            span(
              `class` := "class-name",
              AttrDef("onclick") := s"highlightBlock('${definition.id}')",
              b(definition.representName)
            ),
            ul(definitions.sortBy(_.name).map(recur))
          )
        case JavaEnumClass(_, modifier, _, definitions, _)
            if modifier.access == PUBLIC =>
          li(
            span(
              `class` := "class-name enum-class",
              AttrDef("onclick") := s"highlightBlock('${definition.id}')",
              definition.representName
            ),
            ul(definitions.sortBy(_.name).map(recur))
          )
        case JavaInterface(_, modifier, definitions, _)
            if modifier.access == PUBLIC =>
          li(
            span(
              `class` := "class-name",
        ...
        case JavaAnnotationInterface(
              annotationInterfaceName,
              modifier,
              definitions,
              _
            ) if modifier.access == PUBLIC =>
        ...
        case JavaMethod(modifier, methodName, _, args, _)
            if modifier.access == PUBLIC =>
          li(
            span(
              `class` := "method-name",
...
final case class CodeNonLeaf(name: String, codeNodes: Map[String, CodeNode])
    extends CodeNode {
  ...
  def createHashMap: Map[String, List[JavaDefinition]] = {
    ...
    map.foldLeft(Map.empty[String, List[JavaDefinition]]) {
      case (acc, (_, elem)) =>
        elem.foldLeft(acc) { (acc, m) =>
          acc + ((if (name != "") name + "." else "") + m._1 -> m._2)
        }
    }
  }
  ...
  def buildNavTreeAcc[T](prefix: String): Node[T] = {
    if (codeNodes.size == 1 && codeNodes.values.head.isInstanceOf[CodeNonLeaf])
      codeNodes.values.headOption
        .map {
          case x: CodeNonLeaf =>
            x.buildNavTreeAcc((if (prefix != "") prefix + "." else "") + name)
          case _ => Empty
        }
        .getOrElse(Empty)

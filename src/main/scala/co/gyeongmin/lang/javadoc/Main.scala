...
object Main {
  ...
  def parseArg(
    args: List[String],
    doc: DocumentDescriptionBuilder,
    debug: DebugOptionBuilder
  ): Either[ArgParserError, (DocumentDescription, DebugOption)] = args match {
  ...
  def main(args: Array[String]): Unit =
    parseArg(
      args.toList,
      new DocumentDescriptionBuilder,
      new DebugOptionBuilder
    ) match {
  ...
  def createDoc(
    ...
    val introString = {
      ...
      if (file.exists() && file.length() > 0)
        Transformer
          ...
          .to(HTML)
          .build
          .transform(s"$baseDir/INTRODUCTION.md")
          .toString
      else ""
    }
    ...
    val node =
      goThroughTree(new File(s"$baseDir/src/main/java"), outputDir, debugOption)
  ...
  def goThroughTree(
    ...
    def loop(currentHandle: File, pkgNameAcc: Seq[String]): CodeNode = {
      if (currentHandle.isFile) {
        ...
        log.info(
          s"entered package ${(pkgNameAcc :+ currentHandle.getName).mkString(".")}"
        )
        CodeNonLeaf(
          currentHandle.getName,
          currentHandle
            .listFiles(onlyJavaAndDirFilter)
            .map(x => x.getName -> loop(x, pkgNameAcc :+ currentHandle.getName))
            .toMap
        )
      }
    }

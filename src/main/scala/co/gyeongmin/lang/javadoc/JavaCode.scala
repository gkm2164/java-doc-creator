...
final case class JavaCode(
    packageName: String,
    imports: Vector[String],
    annotationsBuf: Vector[JavaAnnotationCall],
    defs: Vector[JavaDefinition]
) {
  def appendAnnotationBuf(annotation: JavaAnnotationCall): JavaCode =
    copy(annotationsBuf = annotationsBuf :+ annotation)
  ...
  def appendDefinition(javaDefinition: JavaDefinition): JavaCode =
    this.copy(defs = defs :+ javaDefinition)

  def copyWithoutAnnotations: JavaCode =
    JavaCode(packageName, imports, Vector.empty, defs)

  import docgen._

  def show[T](implicit debugOption: DebugOption): Node[T] = div(
    defs.sortBy(_.name).map(_.show(Indent(0)))
  )
}
...
object JavaCode {
  ...
  def emptyCode: JavaCode =
    JavaCode("", Vector.empty, Vector.empty, Vector.empty)

  implicit def nextCodeStateToCodeStateConversion[A](
    nextCodeState: NextCodeState[A]
  ): CodeState[A] =
  ...
  def parseAnnotationName(base: String): CodeState[String] = NextCodeState {
    ...
    case JavaSToken(_, _) :: JavaSToken(DOT, _) :: _ =>
      for {
        annName <- takeString
        _ <- assertToken(DOT)
  ...
  def parseEnum(modifier: JavaModifier): CodeState[JavaEnumClass] = {
    def parseEnum(
      name: String,
      implements: Vector[JavaTypeUse]
    ): CodeState[JavaEnumClass] = {
      ...
      def parseEnumBody(
        tokenName: String,
        acc: Vector[String]
      ): CodeState[JavaEnumClass] = NextCodeState {
        case Nil => throw new TokenNotAcceptedException("nil list")
        case JavaSToken(COMMA, _) :: JavaSToken(SEMICOLON, _) :: _ =>
          for {
            _ <- assertToken(COMMA)
        ...
        case JavaSToken(COMMA, _) :: _ =>
          for {
            _ <- assertToken(COMMA)
            res <- loop(acc :+ tokenName)
          } yield res
        case JavaSToken(SEMICOLON, _) :: _ =>
          for {
            _ <- assertToken(SEMICOLON)
            defs <- parseClassInside(modifier.fullPath, Vector.empty)
          } yield JavaEnumClass(
        ...
        case h :: _ =>
          throw new TokenNotAcceptedException(s"not allowed token: $h")
      }
  ...
  def parseDefs(modifier: JavaModifier): CodeState[JavaDefinition] =
    NextCodeState[JavaDefinition] {
      ...
      case JavaSToken(
            testToken @ (PRIVATE | PROTECTED | PUBLIC | VOLATILE | TRANSIENT |
            SYNCHRONIZED),
            _
          ) :: _ =>
        for {
      ...
      case JavaSToken(STATIC, _) :: JavaSToken(LBRACE, _) :: _ =>
        for {
          _ <- assertToken(STATIC)
          _ <- assertToken(LBRACE)
          _ <- parseParenthesisSkipHead(LBRACE, RBRACE)
        } yield JavaDefinitionUnit
      case JavaSToken(STATIC, _) :: _ =>
        for {
          _ <- assertToken(STATIC)
      ...
      case JavaSToken(FINAL, _) :: _ =>
        for {
          _ <- assertToken(FINAL)
          defs <- parseDefs(modifier.setFinal)
        } yield defs
      case JavaSToken(ABSTRACT, _) :: _ =>
        for {
          _ <- assertToken(ABSTRACT)
          defs <- parseDefs(modifier.setAbstract)
        } yield defs
      case JavaSToken(
            COMMENT_MACRO_CODE | COMMENT_MACRO_EXPLAIN | COMMENT_MACRO_NAME,
            v
          ) :: _ =>
        for {
          _ <- assertTokens(
  ...
  def parseTypeDesignatorRemains(
    acc: Vector[JavaTypeDesignate]
  ): CodeState[Vector[JavaTypeDesignate]] = CodeState {
  ...
  def parseGenerics: CodeState[Vector[JavaTypeDesignate]] =
    NextCodeState[Vector[JavaTypeDesignate]] {
      case JavaSToken(LT, _) :: _ =>
        for {
          _ <- assertToken(LT)
          typeDesignate <- parseTypeDesignator
          ...
          _ <- CodeState {
            ...
            case JavaSToken(RIGHT_SHIFT, _) :: t =>
              (JavaSToken(GT, ">") :: t, ())
            case JavaSToken(U_RIGHT_SHIFT, _) :: t =>
              (JavaSToken(RIGHT_SHIFT, ">>") :: t, ())
            case JavaSToken(v, _) :: _ =>
  ...
  def parseTypeRelation: CodeState[Option[(String, JavaTypeDesignate)]] =
    NextCodeState[Option[(String, JavaTypeDesignate)]] {
      case JavaSToken(EXTENDS | SUPER, _) :: _ =>
        for {
          relation <- takeString
  ...
  def parseArrBrackets(acc: String): CodeState[String] = NextCodeState {
    case JavaSToken(LBRACKET, _) :: JavaSToken(RBRACKET, _) :: _ =>
      for {
  ...
  def takeToken[A](x: A): CodeState[A] =
    State(tokens => (tokens.tail, x)) // 토큰 한개를 흘려보냄

  def parseArgs: CodeState[Vector[JavaArgument]] = CodeState(tokens => {
    def parseArgName(
      typename: JavaTypeUse,
      arg: JavaArgument
    ): CodeState[JavaArgument] =
      State(tokens =>
        (
          tokens.tail,
          arg.copy(argumentType = typename, name = tokens.head.value)
        )
      )

    def parseArg(javaArg: JavaArgument): CodeState[JavaArgument] =
      NextCodeState {
        case JavaSToken(ANNOTATION, _) :: _ =>
          for {
            _ <- assertToken(ANNOTATION)
    ...
    def loop(acc: Vector[JavaArgument]): CodeState[Vector[JavaArgument]] =
      CodeState {
  ...
  def parseMembers(modifier: JavaModifier): CodeState[JavaMembers] = {
    def parseCodes(name: String): CodeState[Vector[JavaSToken]] = CodeState {
      ...
      case h :: _ =>
        throw new TokenNotAcceptedException(s"member: ${h.toString}")
    }

    def parseMemberAfterName(
      modifier: JavaModifier,
      typename: JavaTypeUse,
      name: String
    ): CodeState[JavaMembers] =
      NextCodeState[JavaMembers] {
        case Nil => throw new TokenNotAcceptedException("nil list!")
        case JavaSToken(LEFT_PARENTHESIS, _) :: _ =>
          for {
            _ <- assertToken(LEFT_PARENTHESIS)
            args <- parseArgs
            codes <- parseCodes(name)
          } yield JavaMethod(modifier, name, typename, args, codes)
        case JavaSToken(SUBSTITUTE, _) :: _ =>
          for {
  ...
  def parseClassInside(
    path: String,
    acc: Vector[JavaDefinition]
  ): CodeState[Vector[JavaDefinition]] = NextCodeState {
  ...
  def parseClass(modifier: JavaModifier): CodeState[JavaClass] = for {
    ...
    name,
    modifier.copy(generic = generics),
    defs,
    extendInh,
    implementH
  )
  ...
  def parseTypesFrom(
    targetToken: JavaTokenEnum
  ): CodeState[Vector[JavaTypeUse]] = CodeState {
    case JavaSToken(token, _) :: t if token == targetToken =>
      separateByType.run(t).value
    case tails => (tails, Vector.empty)
  }

  def parseInterfaceDefs(
    path: String,
    acc: Vector[JavaDefinition]
  ): CodeState[Vector[JavaDefinition]] = CodeState {
    case Nil                        => throw new TokenNotAcceptedException("")
  ...
  def parseAnnotationInterface(
    modifier: JavaModifier
  ): CodeState[JavaAnnotationInterface] = for {
    name <- takeString
    extendInh <- parseTypesFrom(EXTENDS)
    _ <- assertToken(LBRACE)
  ...
  def assertTokens(tokens: JavaTokenEnum*): CodeState[JavaTokenEnum] =
    CodeState {
      case JavaSToken(tk, _) :: t if tokens.contains(tk) =>
        State.pure(tk).run(t).value
      case t =>
        throw new TokenNotAcceptedException(
          s"${t.head.tokenType} => ${t.head.value}, ${t.tail.take(5)}"
        )
    }

  def parseParenthesis(
    leftPar: JavaTokenEnum,
    rightPar: JavaTokenEnum
  ): CodeState[String] = {
    def loop(cnt: Int, acc: String): CodeState[String] = NextCodeState {
      ...
      case JavaSToken(lp, _) :: _ if lp == leftPar =>
        for {
          _ <- assertToken(lp)
          l <- loop(cnt + 1, acc + lp.value)
        } yield l
      case JavaSToken(rp, _) :: _ if cnt == 0 && rp == rightPar =>
        for {
          _ <- assertToken(rp)
          res <- State.pure(acc + rp.value)
        } yield res
      case JavaSToken(rp, _) :: _ if rp == rightPar =>
        for {
          _ <- assertToken(rp)
  ...
  def parseParenthesisList(
    leftPar: JavaTokenEnum,
    rightPar: JavaTokenEnum
  ): CodeState[Vector[String]] = {
    def loop(
      cnt: Int,
      works: String,
      acc: Vector[String]
    ): CodeState[Vector[String]] = NextCodeState[Vector[String]] {
  ...
  def takeString: CodeState[String] = CodeState(tokens =>
    (
      tokens.tail,
      if (tokens.head.value.isEmpty) tokens.head.tokenType.value
      else tokens.head.value
    )
  )
  ...
  def assertToken(token: JavaTokenEnum): CodeState[Unit] = CodeState {
    ...
    case t =>
      throw new TokenNotAcceptedException(
        s"expected $token but ${t.head.tokenType} => ${t.head.value}, ${t.tail.take(5)}"
      )
  }

  def parseParenthesisSkipHead(
    leftPar: JavaTokenEnum,
    rightPar: JavaTokenEnum
  ): CodeState[Vector[JavaSToken]] = {
    def loop(cnt: Int, acc: Vector[JavaSToken]): CodeState[Vector[JavaSToken]] =
      NextCodeState[Vector[JavaSToken]] {
        case Nil => State.pure(acc)
        case (t @ JavaSToken(lp, _)) :: _ if lp == leftPar =>
          for {
            _ <- assertToken(lp)
            res <- loop(cnt + 1, acc :+ t)
          } yield res
        case (t @ JavaSToken(rp, _)) :: _ if cnt == 0 && rp == rightPar =>
          for {
            _ <- assertToken(rp)
          } yield acc :+ t
        case (t @ JavaSToken(rp, _)) :: _ if rp == rightPar =>
          for {
            _ <- assertToken(rp)
            res <- loop(cnt - 1, acc :+ t)
          } yield res
        case (t @ JavaSToken(_, _)) :: _ =>
          for {
            _ <- takeString
            res <- loop(cnt, acc :+ t)
          } yield res
  ...
  def parseUntil(until: JavaTokenEnum): CodeState[String] = {
    def loop(acc: String): CodeState[String] = NextCodeState {
      case Nil => State.pure(acc)
      case JavaSToken(target, _) :: _ if target == until =>
        for {
  ...
  def parseUntilDepth(
    until: JavaTokenEnum,
    depthStart: JavaTokenEnum,
    depthEnd: JavaTokenEnum,
    depth: Int = 0
  ): CodeState[String] = {
    def loop(acc: String): CodeState[String] = NextCodeState {
      case Nil => State.pure(acc)
      case JavaSToken(target, _) :: _ if depth == 0 && target == until =>
        for {
          _ <- assertToken(target)
        } yield acc
      case JavaSToken(x, _) :: _ if x == depthStart =>
        for {
          _ <- assertToken(x)
          res <- parseUntilDepth(until, depthStart, depthEnd, depth + 1)
        } yield res
      case JavaSToken(x, _) :: _ if x == depthEnd =>
        for {
          _ <- assertToken(x)
          res <- parseUntilDepth(until, depthStart, depthEnd, depth - 1)
        } yield res
  ...
  def parseCode(bowl: JavaCode): CodeState[JavaCode] = NextCodeState {
    ...
    case _ =>
      for {
        definition <- parseDefs(
          JavaModifier.empty(bowl.packageName, bowl.annotationsBuf)
        )
        code <- parseCode(
          bowl.appendDefinition(definition).copyWithoutAnnotations
        )
      } yield code
  }
  ...
  def apply(tokens: List[JavaSToken]): JavaCode =
    parseCode(emptyCode)
      .runA(
        tokens.filterNot(x =>
          List(COMMENT, COMMENT_BLOCK).contains(x.tokenType)
        )
      )
      .value
  ...
  object CodeState {
    ...
    def apply[A](f: List[JavaSToken] => (List[JavaSToken], A)): CodeState[A] =
      State(f)
  }

      ...
      div(id := s"def-${definition.id}", `class` := "definition-box", style := s"margin-left: ${indent.ch()}px;",
        a(id := definition.id), definition match {
          case c@JavaClass(name, modifier, definitions, inheritClass, implementInterfaces) =>
            div(if (c.representName != name) h6(`class` := "class-role", c.representName) else Empty,
              h5(`class` := "java-def class-def",
                s" $name",
                if (modifier.generic.nonEmpty) span(
                  span(`class` := "generic-symbol", "&lt;"),
                  modifier.generic.map(x => span(x.show)),
                  span(`class` := "generic-symbol", "&gt;")
                ) else Empty),
              if (inheritClass.nonEmpty) span(span(`class` := "reserved-keyword", " extends "), inheritClass
                .map(x => x.show).mkString(", "))
              else Empty,
              if (implementInterfaces
                .nonEmpty) span(span(`class` := "reserved-keyword", " implements "), implementInterfaces
                .map(x => x.show).mkString(", "))
              else Empty,
          ...
          case JavaEnumClass(name, _, enumTokens, definitions, _) =>
            val publicDefinitions = definitions.filter(_.modifier.access != PRIVATE).sortBy(_.name)

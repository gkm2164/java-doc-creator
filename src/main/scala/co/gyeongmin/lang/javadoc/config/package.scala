...
package object config {
  ...
  implicit def partialSettingKeyImplicit[Target]: Monad[SettingKey[*, Target]] =
    new Monad[SettingKey[*, Target]] {
      override def flatMap[A, B](fa: SettingKey[A, Target])(
        f: A => SettingKey[B, Target]
      ): SettingKey[B, Target] = new SettingKey[B, Target] {
        ...
        override def binder(x: Target, v: B): Target =
          nextSetter.binder(fa.binder(x, currentValue), v)
      }

      @scala.annotation.tailrec
      override def tailRecM[A, B](
        a: A
      )(f: A => SettingKey[Either[A, B], Target]): SettingKey[B, Target] =
        f(a).getOrDefault match {
          case Left(v)  => tailRecM(v)(f)
          case Right(v) => pure(v)
        }

      override def pure[A](v: A): SettingKey[A, Target] =
        SettingKey((x, _) => x, v)
    }
  ...
  implicit class SettingKeyMonadSyntax[A, Target](fa: SettingKey[A, Target]) {
    ...
    def flatMap[B](f: A => SettingKey[B, Target]): SettingKey[B, Target] =
      F.flatMap(fa)(f)
  ...
  case class DocumentDescription(
      baseDir: String,
      outputDir: String,
      description: String
  ) extends DescriptorType

  case class DebugOption(
      stackTrace: Boolean,
      maxStackSize: Int,
  ...
  class DocumentDescriptionBuilder extends BuilderType[DocumentDescription] {
    ...
    val description: SettingKey[String, DocumentDescription] =
      SettingKey(
        (self, value) => self.copy(description = value),
        "default description"
      )
    val empty: DocumentDescription = DocumentDescription("", "", "")

    protected val combine: List[SettingKey[_, DocumentDescription]] =
      baseDir :: outputDir :: description :: Nil
  }
  ...
  class DebugOptionBuilder extends BuilderType[DebugOption] {
    ...
    val empty: DebugOption =
      DebugOption(stackTrace = false, 0, printOnlyAccepted = false)
    protected val combine: List[SettingKey[_, DebugOption]] =
      stackTrace :: maxStackSize :: printOnlyAccepted :: Nil
  }

  object SettingKey {
    def apply[T, Target](
      givenBinder: (Target, T) => Target,
      givenDefaultValue: T
    ): SettingKey[T, Target] = new SettingKey[T, Target] {

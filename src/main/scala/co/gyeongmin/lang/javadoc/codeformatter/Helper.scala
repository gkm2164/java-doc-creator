...
object Helper {
  ...
  def consumeTokens(enums: Set[JavaTokenEnum]): CodeWriter[JavaSToken] = CodeWriter {
    case Nil => (Nil, Left(TokenListEmptyError()))
  ...
  def takeToken(enums: Set[JavaTokenEnum], converter: JavaSToken => String = _.value): CodeWriter[String] = tag(CodeWriter {
    case Nil => (Nil, Left(TokenListEmptyError()))
  ...
  }, s"takeTokens(${enums.mkString(" | ")})").hint(enums)

  def tag[A](writer: CodeWriter[A], t: String): CodeWriter[A] = writer.pushTag(t)

  def assertToken(enum: JavaTokenEnum): CodeWriter[Unit] = tag(takeToken(enum).map(_ => ()), s"assertToken($enum)")

  def assertToken(enums: Set[JavaTokenEnum]): CodeWriter[Unit] = tag(takeToken(enums).map(_ => ()), s"assertTokens(${enums.mkString(" | ")})")

...
package object javadoc {
  ...
  val ASCIIValues: IndexedSeq[Char] =
  ...
  def escapeLTGT(str: String): String = { // 문서 상에 <, > 부호가 안보이는 경우...
    ...
    def loop(str: String, acc: String): String = {
      if (str.isEmpty) acc
        ...
        loop(str.tail, acc + """<span class="generic-symbol">&lt;</span>""")
      else if (str.head == '>')
        loop(str.tail, acc + """<span class="generic-symbol">&gt;</span>""")
  ...
  sealed trait JavaDefinition {
    ...
    final def id: String = {
      val ret =
        s"${modifier.fullPath.toLowerCase.replace(".", "-")}-${name.toLowerCase}"
      this match {
        case JavaMethod(_, _, _, args, _) if args.nonEmpty =>
          ret + "-" + args.map(_.name.toLowerCase).mkString("-")
        case _ => ret
  ...
  final case class JavaModifier(
      commentMacros: Vector[String],
    ...
    def show[T]: Node[T] = span(
      ...
      annotations.map(x =>
        span(`class` := "annotation-use", s"@${x.name}${x.parameters} ")
      ),
      span(`class` := "reserved-keyword", s"${access.value} "),
      Empty,
      if (isStatic) span(`class` := "reserved-keyword", "static ") else Empty,
      if (isFinal) span(`class` := "reserved-keyword", "final ") else Empty,
      if (isAbstract) span(`class` := "reserved-keyword", "abstract ")
      else Empty
    )

    def appendMacro(v: String): JavaModifier =
      this.copy(commentMacros = commentMacros :+ v)

    def appendAnnotation(annotation: JavaAnnotationCall): JavaModifier = this
      .copy(annotations = annotations :+ annotation)

    def setAccess(access: JavaTokenEnum): JavaModifier =
      this.copy(access = access)

    def setGeneric(generic: Vector[JavaTypeDesignate]): JavaModifier =
      this.copy(generic = generic)
    ...
    def setPath(path: String): JavaModifier =
      this.copy(fullPath = fullPath + path)
  }

  final case class JavaEnumClass(
      name: String,
  ...
  final case class JavaClass(
      name: String,
      modifier: JavaModifier,
      definitions: Vector[JavaDefinition],
      inheritClass: Vector[JavaTypeUse],
      implementInterfaces: Vector[JavaTypeUse]
  ) extends JavaTypeDef {

    lazy val exampleCodes: Vector[String] =
      modifier.commentMacros.filter(_.startsWith("//="))
  }

  final case class JavaInterface(
      name: String,
      modifier: JavaModifier,
  ...
  final case class JavaAnnotationInterface(
      name: String,
      modifier: JavaModifier,
      definitions: Vector[JavaDefinition],
      inheritClass: Vector[JavaTypeUse]
    ...
    val annotationMeta: Map[String, Vector[String]] =
      modifier.annotations
        ...
        .toMap
    val decideTarget: Option[String] =
      annotationMeta.get("Target").map(_.headOption.getOrElse(""))

    override def implementInterfaces: Vector[JavaTypeUse] = Vector.empty

  }

  final case class JavaMember(
      modifier: JavaModifier,
      name: String,
      memberType: JavaTypeUse
  ) extends JavaMembers {
  ...
  final case class JavaMethod(
      modifier: JavaModifier,
      name: String,
      returnType: JavaTypeUse,
      args: Vector[JavaArgument],
      codes: Vector[JavaSToken]
  ) extends JavaMembers {
    lazy val exampleCode: Vector[String] =
      modifier.commentMacros.filter(_.startsWith("//="))
  }

  final case class JavaTypeDesignate(
      name: String,
      extend: Option[(String, JavaTypeDesignate)],
      generics: Vector[JavaTypeDesignate],
    ...
    def show: String =
      renderHtml(showNode, TextPrettyPrintingConfig.noPrettyPrinting)

    def genericSymbol[T](word: String): Node[T] =
      span(`class` := "generic-symbol", word)

    def describeGenerics[T]: Node[T] = span(
      genericSymbol("&lt;"),
      generics
        .map(x =>
          renderHtml(x.showNode, TextPrettyPrintingConfig.noPrettyPrinting)
        )
        .mkString(", "),
      genericSymbol("&gt;")
    )
    ...
    def showNode[T]: Node[T] = {
      if (
        Vector("boolean", "void", "int", "double", "short", "char")
          .contains(name)
      ) Seq(span(`class` := "reserved-keyword", name + arrayNotation))
      else
        Seq(
          span(`class` := "type-keyword", name),
          if (generics.nonEmpty) describeGenerics else Empty,
          span(arrayNotation)
        )
    }
  }

  final case class JavaTypeUse(
      typeDesignate: JavaTypeDesignate,
  ...
  final case class JavaArgument(
      annotations: Vector[JavaAnnotationCall],
      isFinal: Boolean,
      name: String,
      argumentType: JavaTypeUse
  ) {
    def setFinal(): JavaArgument = copy(isFinal = true)

    def appendAnnotation(annotation: JavaAnnotationCall): JavaArgument =
      copy(annotations = annotations :+ annotation)
    ...
    def show[T]: Node[T] = span(
      if (annotations.nonEmpty)
        annotations
          .map(x =>
            span(`class` := "annotation-def", s"@${x.name}${x.parameters} ")
          )
      else Empty,
      if (isFinal) span(`class` := "reserved-keyword", "final ") else Empty,
  ...
  object JavaTypeUse {
    def empty: JavaTypeUse =
      JavaTypeUse(JavaTypeDesignate("", None, Vector.empty, ""), "")
  }

  object JavaModifier {
    def empty(
      path: String,
      annotationsBuf: Vector[JavaAnnotationCall] = Vector.empty
    ): JavaModifier =
      JavaModifier(
        Vector.empty,
        annotationsBuf,
  ...
  object VectorImplicit {
    ...
    implicit class strVectorExt[+A](list: Vector[A]) {
      def addIf[B >: A](p: => Boolean, item: B): Vector[B] =
        if (p) list :+ item else list
  ...
  object JavaArgument {
    def empty: JavaArgument =
      JavaArgument(Vector.empty, isFinal = false, "", JavaTypeUse.empty)
  }
